...
import {Info,Euro,DollarSign,BadgePoundSterling } from "lucide-react";
...
function CryptoCurse() {
  ...
  return (
     ...
     exchange?.map(({code,rate,description})=>{
        return(
            <Card className="ml-4" style={{ minWidth: "260px" }} key={rate}>
            <CardHeader className="flex-row item-center p-2 pl-0 ml-4">
            ...
            <CardFooter className="flex justify-between p-2 ml-4 mr-4 mb-2">
              <Button size="icon">
                <Info size={48} />
              </Button>
